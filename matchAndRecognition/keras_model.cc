...
void keras::DataChunk2D::read_from_image(cv::Mat image)
{
	m_depth = image.channels();
	m_rows = image.rows;
	m_cols = image.cols;
	...
	if (m_depth == 3)
	{
		for (int d = 0; d < m_depth; ++d)
		{
			...
			for (int r = 0; r < m_rows; ++r)
			{
				...
				{
					float col_data = (int)image.at<Vec3b>(r, c)[m_depth-d-1];
					tmp_row.push_back(col_data);
				}
				tmp_single_depth.push_back(tmp_row);
			}
			data.push_back(tmp_single_depth);
	...
	if (m_depth == 1)
	{
		for (int d = 0; d < m_depth; ++d)
		{
			...
			for (int r = 0; r < m_rows; ++r)
			{
				...
				for (int c = 0; c < m_cols; ++c)
				{
					float col_data = (int)image.at<uchar>(r, c);
					tmp_row.push_back(col_data);
				}
				tmp_single_depth.push_back(tmp_row);
			}
...
keras::DataChunk* keras::LayerDense::compute_output(keras::DataChunk* dc) 
{
  ...
  for (unsigned int j = 0; j < m_weights.size(); ++j) { 
    for (unsigned int i = 0; i < m_weights[j].size(); ++i) { 
      y_ret[i] += m_weights[j][i] * im[j];
    }
  }
  for (unsigned int i = 0; i < m_weights[0].size(); ++i) { 
    y_ret[i] += m_bias[i];
  }
